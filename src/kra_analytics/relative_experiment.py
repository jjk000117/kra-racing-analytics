# Long contract and artifact paths remain readable as complete expressions.
# ruff: noqa: E501
...
from kra_analytics.relative_features import (
    CANDIDATE_TABLE,
    RELATIVE_FEATURES,
    relative_candidate_hash,
    relative_feature_hash,
)
from kra_analytics.trend_experiment import _contracts as trend_contracts

EXPERIMENT_VERSION = "post_baseline_v2_relative_r1_development_v1"
CANDIDATES = ("LT1", "LR1")
EXPECTED_LT1_HASH = "1dcf5f5a630d67f216fa6dc27932d2c6532ec0a50af6360b7521917682f802e8"
EXPECTED_R1_HASH = "f058c627fb68960803ed88adf450e67796fc2df25e8ad8854c372374d8875539"
EXPECTED_LR1_HASH = "7fec6229b3b355d34e664823407765c9a597eacdafa11a733048ba2eaff1a85a"
...
def _contracts(paths: ProjectPaths) -> dict[str, V2FeatureContract]:
    ...
    if relative_feature_hash(paths) != EXPECTED_R1_HASH:
        raise ValueError("Sealed R1 hash mismatch")
    inputs = lt1.inputs + RELATIVE_FEATURES
    lr1 = V2FeatureContract(
        inputs=inputs,
        categorical=lt1.categorical,
        numeric=lt1.numeric + RELATIVE_FEATURES,
        zero_count=lt1.zero_count,
        feature_hash=relative_candidate_hash(paths),
    )
    if len(inputs) != len(set(inputs)) or len(inputs) != 144:
        raise ValueError("LR1 must contain exactly 144 unique Features")
    if lr1.feature_hash != EXPECTED_LR1_HASH:
        raise ValueError("Sealed LR1 hash mismatch")
    return {"LT1": lt1, "LR1": lr1}
...
def _load_frame(paths: ProjectPaths, contract: V2FeatureContract) -> pd.DataFrame:
    ...
    columns = list(dict.fromkeys(("race_id", "horse_id", "race_date", *contract.inputs, TARGET_COLUMN)))
...
def _fit(train: pd.DataFrame, evaluation: pd.DataFrame, contract: V2FeatureContract) -> tuple[dict[str, float], Any, float, list[str]]:
    ...
    return _metric_payload(evaluation, probability), pipeline, elapsed, [str(item.message) for item in caught]


def _preprocessing(fold_id: str, train: pd.DataFrame, evaluation: pd.DataFrame, pipeline: Any) -> list[dict[str, Any]]:
    preprocessor = pipeline.named_steps["preprocessor"]
    numeric_names = list(preprocessor.transformers_[1][2])
    statistics = dict(zip(numeric_names, preprocessor.named_transformers_["numeric"].named_steps["imputer"].statistics_, strict=True))
    rows: list[dict[str, Any]] = []
    for feature in RELATIVE_FEATURES:
        train_values = pd.to_numeric(train[feature], errors="coerce").astype("float64")
        fitted = float(statistics[feature])
        expected = float(train_values.median())
    ...
    return [
        {
            "fold_id": fold_id,
            "feature_name": feature,
            "standardized_coefficient": float(mapping[f"numeric__{feature}"]),
            "sign": "positive" if mapping[f"numeric__{feature}"] > 0 else "negative" if mapping[f"numeric__{feature}"] < 0 else "zero",
        }
        for feature in RELATIVE_FEATURES
    ]
...
def _summaries(metrics: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for candidate in CANDIDATES:
        selected = [row for row in metrics if row["experiment_id"] == candidate]
...
def _deltas(metrics: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for fold in DEVELOPMENT_FOLDS:
        control = next(row for row in metrics if row["experiment_id"] == "LT1" and row["fold_id"] == fold.fold_id)
        candidate = next(row for row in metrics if row["experiment_id"] == "LR1" and row["fold_id"] == fold.fold_id)
        result: dict[str, Any] = {"fold_id": fold.fold_id}
        for metric in METRICS:
            delta = float(candidate[metric]) - float(control[metric])
            result[f"delta_{metric}"] = delta
            result[f"relative_delta_{metric}"] = delta / float(control[metric])
        rows.append(result)
    return rows


def _decision(summaries: list[dict[str, Any]], deltas: list[dict[str, Any]]) -> dict[str, Any]:
    control = next(row for row in summaries if row["experiment_id"] == "LT1")
    candidate = next(row for row in summaries if row["experiment_id"] == "LR1")
    ll = float(candidate["macro_log_loss_mean"]) - float(control["macro_log_loss_mean"])
    brier = float(candidate["macro_brier_mean"]) - float(control["macro_brier_mean"])
    ...
    both = sum(float(row["delta_macro_log_loss"]) < 0 and float(row["delta_macro_brier"]) < 0 for row in deltas)
    keep = ll < 0 and brier < 0 and ll_folds >= 3 and brier_folds >= 3 and both >= 3
    return {
        "judgement": "KEEP_R1" if keep else "DROP_R1",
        "delta_macro_log_loss_mean": ll,
        "relative_delta_macro_log_loss_mean": ll / float(control["macro_log_loss_mean"]),
        "delta_macro_brier_mean": brier,
        "relative_delta_macro_brier_mean": brier / float(control["macro_brier_mean"]),
...
def run_r1_development_experiment(paths: ProjectPaths | None = None) -> dict[str, Any]:
    ...
    protected = {
        ...
        "r1_audit": project.root / "data/exports/validation/post_baseline_v2_relative_r1/audit_result.json",
        "a1_result": project.root / "data/exports/modeling/post_baseline_v2_aptitude_a1_development_v1/result.json",
        "t1_result": common / REFERENCE_RESULT,
        "validation_contract": project.root / "docs/post-baseline-v2-improvement-validation-contract.json",
        "validation_ledger": common / "data/exports/modeling/post_baseline_v2_f1_f3_one_time_validation_v1/validation_access.json",
        "validation_result": common / "data/exports/modeling/post_baseline_v2_f1_f3_one_time_validation_v1/result.json",
        "h133_result": common / "data/exports/modeling/post_baseline_v2_h133_development_v1/result.json",
        "ra1_result": common / "data/exports/modeling/post_baseline_v2_ra1_development_v1/result.json",
    }
    ...
    for spec in DEVELOPMENT_FOLDS:
        ...
        for candidate_name in CANDIDATES:
            values, pipeline, elapsed, caught = _fit(train, evaluation, contracts[candidate_name])
            metrics.append(
                {
                    "experiment_id": candidate_name,
                    "fold_id": spec.fold_id,
                    **values,
                    "fit_seconds": elapsed,
    ...
    reference = next(item["summary"] for item in reference_payload["candidates"] if item["experiment_id"] == "LT1")
    reproduced = next(item for item in summaries if item["experiment_id"] == "LT1")
    reproduction = {metric: float(reproduced[f"{metric}_mean"]) - float(reference[f"{metric}_mean"]) for metric in METRICS}
    ...
    for fold in DEVELOPMENT_FOLDS:
        evaluation = _fold_frames(frame, fold)[1]
        availability_by_fold.append(
            {
                "fold_id": fold.fold_id,
                "mean_r1_availability": float(evaluation[list(RELATIVE_FEATURES)].notna().mean().mean()),
                "delta_macro_log_loss": next(row["delta_macro_log_loss"] for row in deltas if row["fold_id"] == fold.fold_id),
                "delta_macro_brier": next(row["delta_macro_brier"] for row in deltas if row["fold_id"] == fold.fold_id),
            }
        )
    availability_vector = np.asarray([row["mean_r1_availability"] for row in availability_by_fold])
    ll_vector = np.asarray([row["delta_macro_log_loss"] for row in availability_by_fold])
    brier_vector = np.asarray([row["delta_macro_brier"] for row in availability_by_fold])
    ...
    result = {
        ...
        "fold_deltas": deltas,
        "r1_preprocessing_audit": preprocessing,
        "r1_coefficients": coefficients,
        "r1_coefficient_summary": coefficient_summary,
        "availability_diagnostic": {
            "folds": availability_by_fold,
            "availability_delta_macro_log_loss_correlation": float(np.corrcoef(availability_vector, ll_vector)[0, 1]),
            "availability_delta_macro_brier_correlation": float(np.corrcoef(availability_vector, brier_vector)[0, 1]),
            "interpretation_limit": "Four-fold descriptive association only; not causal and not a selection rule.",
        },
    ...
    for filename, rows in (
        ...
        ("r1_preprocessing_audit.csv", preprocessing),
        ("r1_coefficients.csv", coefficients),
        ("r1_coefficient_summary.csv", coefficient_summary),
        ("r1_availability_diagnostic.csv", availability_by_fold),
    ):
        pd.DataFrame(rows).to_csv(output / filename, index=False)
    for filename in ("experiment_registry.json", "result.json"):
