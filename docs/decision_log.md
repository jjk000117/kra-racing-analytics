...
## 2026-08-18 — 개발기간 접근 경계와 Validation 접근 예산을 코드로 강제

결정:

- 개선 실험 loader는 2023-01-01~2024-06-30만 읽고 날짜 인자를 외부에 노출하지 않는다.
- 2024-07-01 이후를 포함하는 요청은 SQL 실행 전에 거부한다.
- 후보는 네 development fold 지표를 모두 기록한 뒤에만 Validation용으로 봉인할 수 있다.
- 봉인 후보별 Validation 접근은 1회로 제한하고 횟수와 사유를 registry에 남긴다.
...
## 2026-08-18 — Development에서 F1·F3 유지, F2 제외

결정:

- F1과 F3를 후속 조합 실험 후보로 유지한다.
- F2는 후속 후보에서 제외하고 정의나 window를 결과에 맞춰 수정하지 않는다.
- 다음 조합 실험은 F1+F3 한 가지로 제한한다.

이유:

- F1과 F3는 Macro Log Loss와 Macro Brier를 모두 4/4 fold에서 개선했다.
- F2는 두 주요 지표 모두 0/4 fold 개선으로 평균도 악화했다.
- F1은 개선 폭이 작으므로 F3와 함께 쓸 때 독립적인 추가 가치가 남는지 확인할 필요가 있다.

## 2026-08-18 — F1+F3를 개발단계 최종 후보로 선택

결정:

- 기존 117개에 F1 6개와 F3 10개를 더한 133개 입력 Logistic을 Validation 후보로 선택한다.
- F2와 추가 Feature 조합은 개발단계에서 더 탐색하지 않는다.
