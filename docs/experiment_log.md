...
- Development current rows 28,392행·2,675경주에서 target 관계나 모델 성능 없이 과거 관측 수만 확인했다.
- 동일등급, 동일 주로상태, 함수율 ±5%p, 동일 날씨, 동일/유사거리 F1, 직전 거리 가용성을 비교했다.

### 결과와 해석

- 최소 3회 가용률은 동일등급 50.95%, 동일 주로상태 39.13%, 동일거리 F1 42.26%였다.
- 직전 거리는 94.40%에서 확인됐고 동일거리/연장/단축 비중은 46.78%/31.99%/21.23%였다.
- 함수율·날씨·유사거리는 가용성이 더 높지만 첫 Aptitude 질문과의 중복 또는 정의 임의성 때문에 제외했다.

### 다음 실험 아이디어

- source DB read-only와 branch-local output을 분리한 뒤 A1 7개를 구현·감사한다.
- 이후 동일 Logistic development fold에서 LT1 137 대 LA1 144만 비교한다.
...
## 2026-08-18 — F1+F3 Logistic development CV

### 실험 내용

- B0, F1, F3와 유일한 조합 F1+F3를 동일 네 expanding fold에서 다시 실행했다.
- 모델·전처리·기간을 고정하고 F1의 F3 초과 기여만 확인했다.

### 결과와 해석

- F1+F3 평균 Macro Log Loss/Brier는 0.527685/0.175744였다.
- F3 대비 평균 차이는 -0.001179/-0.000459이며 두 지표 모두 4/4 fold 개선했다.
- F1+F3를 개발단계 최종 후보로 선택했다. Validation 접근은 0회였다.
...
## 2026-09-08 — Aptitude A1 데이터 수준 감사

### 실험 내용

- 봉인된 A1 7개를 development 28,392행·2,675경주에 계산했다.
- 연도·경마장·temporal fold별 가용성과 분포, PIT, count/value 및 10건 수동 재계산을 감사했다.
- target 관계나 모델 성능은 계산하지 않았다.

### 결과와 해석

- rate/median 가용률은 동일등급 50.95%, 동일 주로상태 39.13%, 동일거리 F1 42.26%였다.
- 직전 거리 변화는 94.40%에서 계산 가능했고 연장/동일/단축이 모두 존재했다.
- 감사 이슈와 보호 artifact 변경은 0건이므로 LA1 개발 실험을 수행할 데이터 blocker는 없다.
...
## 2026-09-09 — LT1 대 LR1 Development CV

### 실험 내용

- 기존 LT1 137과 봉인된 R1 7개를 더한 LR1 144를 동일 raw Logistic으로 비교했다.
- Development 28,392행·2,675경주와 기존 네 expanding fold만 사용했다.

### 결과와 해석

- Macro Log Loss 평균은 0.527003 대 0.526061, Macro Brier는 0.175484 대 0.175132였다.
- 두 primary 지표 동시 개선은 fold 1, 2, 4의 3/4였다.
- `KEEP_R1`: 개선은 시간적으로 반복됐지만 상대 폭은 0.18%~0.20%로 작다.
- R1 7개 표준화 계수는 각각 네 fold에서 부호가 일관됐지만 이는 다변량 조건부 관계다.

### 다음 실험 아이디어

- Validation을 다시 사용하거나 공식 후보로 승격하려면 별도의 사전 봉인 계약이 필요하다.

## 2026-09-10 — LT1 대 LR1 Validation 재현 검사

### 실험 내용

- 기존에 노출된 2024-07~2025-06 Validation에서 LT1 137과 LR1 144를 동일 Logistic·OOF sigmoid로 비교했다.
- 원천 SQL 파싱·바인딩 단계의 실패 2건은 별도 ledger로 보존했고, 승인된 재개에서 실제 평가를 완료했다.
