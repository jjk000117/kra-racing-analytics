...
### 5E: 공식 적중배당 정규화 설계와 프로파일링

1. `confirmed_odds_raw`의 승식별 문자열 형식을 프로파일링한다.
2. 공식 적중 조합과 확정배당의 목표 Grain을 승식별로 정의한다.
3. 단승·연승·복승·쌍승·복연승·삼복승·삼쌍승 파싱 규칙을 설계한다.
4. 파싱 성공률, 누락, 중복 조합과 극단값을 검사한다.
5. 정규화 결과를 근거로 승식 후보 선정 기준을 다시 검토한다.
